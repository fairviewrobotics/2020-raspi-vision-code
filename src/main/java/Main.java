...
public final class Main {
  ...
  public static class HighGoalPipeline implements VisionPipeline {
    static NetworkTableInstance inst = NetworkTableInstance.getDefault();
    static NetworkTable table;
    static NetworkTableEntry yaw;
    static NetworkTableEntry targetFound;
    static NetworkTableEntry targetAtTopImage;

    static {
      table = inst.getTable("high-vision");
      yaw = table.getEntry("yaw");
      targetFound = table.getEntry("isTarget");
      targetAtTopImage = table.getEntry("targetAtTopImage");
    }

    public HighGoalPipeline() {
    }

    public void process(Mat mat) {
      List<TargetLocation> targets = HighGoalVision.highGoalDetect(
              mat,
              new Size(320.0, 240.0),
              new Scalar(53.0, 213.0, 100.0),
              new Scalar(100.0, 255.0, 255.0),
              1,
              5,
              0.001,
              Math.toRadians(68.5),
      ...
      // only pick first (largest) target found
      if (targets.size() >= 1) {
        yaw.setDouble((targets.get(0)).yaw);
        // if target is in top 1/4 of image, mark it as at top (so robot knows to stop)
        targetAtTopImage.setBoolean(targets.get(0).y < 0.25);
